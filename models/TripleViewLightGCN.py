...
class TripleViewLightGCN(nn.Module):
    def __init__(self, conf, raw_graph):
        ...
        # All graphs are propagation graphs (LightGCN style)
    ...
    def get_multi_modal_representations(self, test=False):
        ...
        # 3. BI LightGCN (Note: This is propagation, not aggregation)
        BI_bundles_rep, BI_items_rep = self.propagate(BI_graph, self.bundles_feature, self.items_feature)

        # 4. Fusion (Concatenation)
        # users: [e_u^UI | e_u^UB]
        users_rep = torch.cat([UI_users_rep, UB_users_rep], dim=1)
    ...
    def forward(self, batch, ED_drop=False):
        ...
        # users_embedding: [bs, 1+neg_num, 2*dim]
        users_embedding = users_rep[users].expand(-1, bundles.shape[1], -1)
        
        # bundles_embedding: [bs, 1+neg_num, 2*dim]
    ...
    def evaluate(self, propagate_result, users):
        ...
        batch_users_emb = users_rep[users] # [batch_size, 2*dim]
